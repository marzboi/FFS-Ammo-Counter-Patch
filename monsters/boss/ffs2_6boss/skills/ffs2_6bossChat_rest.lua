...
function ffs2_6bossChat_rest.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_rest.projectile.type"), config.getParameter("ffs2_6bossChat_rest.projectile.config"), config.getParameter("ffs2_6bossChat_rest.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_rest.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_rest.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_rest.skillTime"),
...
function ffs2_6bossChat_rest.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_rest.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_rest.projectileOffset")
...
function ffs2_6bossChat_rest.leavingState(stateData)
end
