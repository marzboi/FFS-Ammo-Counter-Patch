...
function ffs2_6bossChat_3.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_3.projectile.type"), config.getParameter("ffs2_6bossChat_3.projectile.config"), config.getParameter("ffs2_6bossChat_3.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_3.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_3.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_3.skillTime"),
...
function ffs2_6bossChat_3.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_3.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_3.projectileOffset")
...
function ffs2_6bossChat_3.leavingState(stateData)
end
