...
function ffs2_6boss1Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6boss1Attack.projectile.type"), config.getParameter("ffs2_6boss1Attack.projectile.config"), config.getParameter("ffs2_6boss1Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6boss1Attack.bobTime"),
    bobHeight = config.getParameter("ffs2_6boss1Attack.bobHeight"),
    skillTime = config.getParameter("ffs2_6boss1Attack.skillTime"),
...
function ffs2_6boss1Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6boss1Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "1Attack")
 
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6boss1Attack.projectileOffset")
...
function ffs2_6boss1Attack.leavingState(stateData)
end
