...
function ffs2_6bossChat_2.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_2.projectile.type"), config.getParameter("ffs2_6bossChat_2.projectile.config"), config.getParameter("ffs2_6bossChat_2.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_2.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_2.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_2.skillTime"),
...
function ffs2_6bossChat_2.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_2.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_2.projectileOffset")
...
function ffs2_6bossChat_2.leavingState(stateData)
end
