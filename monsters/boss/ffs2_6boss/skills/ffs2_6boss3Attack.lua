...
function ffs2_6boss3Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6boss3Attack.projectile.type"), config.getParameter("ffs2_6boss3Attack.projectile.config"), config.getParameter("ffs2_6boss3Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6boss3Attack.bobTime"),
    bobHeight = config.getParameter("ffs2_6boss3Attack.bobHeight"),
    skillTime = config.getParameter("ffs2_6boss3Attack.skillTime"),
...
function ffs2_6boss3Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6boss3Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "3Attack")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6boss3Attack.projectileOffset")
...
function ffs2_6boss3Attack.leavingState(stateData)
end
