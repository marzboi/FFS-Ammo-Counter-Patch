...
function ffs2_6bossChat_6.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_6.projectile.type"), config.getParameter("ffs2_6bossChat_6.projectile.config"), config.getParameter("ffs2_6bossChat_6.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_6.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_6.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_6.skillTime"),
...
function ffs2_6bossChat_6.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_6.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_6.projectileOffset")
...
function ffs2_6bossChat_6.leavingState(stateData)
end
