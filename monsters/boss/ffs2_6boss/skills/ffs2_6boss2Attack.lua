...
function ffs2_6boss2Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6boss2Attack.projectile.type"), config.getParameter("ffs2_6boss2Attack.projectile.config"), config.getParameter("ffs2_6boss2Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6boss2Attack.bobTime"),
    bobHeight = config.getParameter("ffs2_6boss2Attack.bobHeight"),
    skillTime = config.getParameter("ffs2_6boss2Attack.skillTime"),
...
function ffs2_6boss2Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6boss2Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "2Attack")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6boss2Attack.projectileOffset")
...
function ffs2_6boss2Attack.leavingState(stateData)
end
