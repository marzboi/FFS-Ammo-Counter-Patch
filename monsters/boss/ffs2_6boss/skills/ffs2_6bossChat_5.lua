...
function ffs2_6bossChat_5.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_5.projectile.type"), config.getParameter("ffs2_6bossChat_5.projectile.config"), config.getParameter("ffs2_6bossChat_5.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_5.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_5.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_5.skillTime"),
...
function ffs2_6bossChat_5.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_5.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_5.projectileOffset")
...
function ffs2_6bossChat_5.leavingState(stateData)
end
