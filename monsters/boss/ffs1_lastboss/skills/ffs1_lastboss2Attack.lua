--------------------------------------------------------------------------------
ffs1_lastboss2Attack = {}

function ffs1_lastboss2Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastboss2Attack.projectile.type"), config.getParameter("ffs1_lastboss2Attack.projectile.config"), config.getParameter("ffs1_lastboss2Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastboss2Attack.bobTime"),
    bobHeight = config.getParameter("ffs1_lastboss2Attack.bobHeight"),
    skillTime = config.getParameter("ffs1_lastboss2Attack.skillTime"),
...
function ffs1_lastboss2Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastboss2Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "2Attack")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastboss2Attack.projectileOffset")
...
function ffs1_lastboss2Attack.leavingState(stateData)
end
