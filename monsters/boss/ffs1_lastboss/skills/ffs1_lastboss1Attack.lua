--------------------------------------------------------------------------------
ffs1_lastboss1Attack = {}

function ffs1_lastboss1Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastboss1Attack.projectile.type"), config.getParameter("ffs1_lastboss1Attack.projectile.config"), config.getParameter("ffs1_lastboss1Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastboss1Attack.bobTime"),
    bobHeight = config.getParameter("ffs1_lastboss1Attack.bobHeight"),
    skillTime = config.getParameter("ffs1_lastboss1Attack.skillTime"),
...
function ffs1_lastboss1Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastboss1Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "1Attack")
 
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastboss1Attack.projectileOffset")
...
function ffs1_lastboss1Attack.leavingState(stateData)
end
