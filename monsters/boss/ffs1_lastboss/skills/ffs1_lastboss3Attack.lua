--------------------------------------------------------------------------------
ffs1_lastboss3Attack = {}

function ffs1_lastboss3Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastboss3Attack.projectile.type"), config.getParameter("ffs1_lastboss3Attack.projectile.config"), config.getParameter("ffs1_lastboss3Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastboss3Attack.bobTime"),
    bobHeight = config.getParameter("ffs1_lastboss3Attack.bobHeight"),
    skillTime = config.getParameter("ffs1_lastboss3Attack.skillTime"),
...
function ffs1_lastboss3Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastboss3Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "2Attack")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastboss3Attack.projectileOffset")
...
function ffs1_lastboss3Attack.leavingState(stateData)
end
