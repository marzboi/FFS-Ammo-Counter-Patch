--------------------------------------------------------------------------------
ffs1_lastbossDamaged2 = {}

function ffs1_lastbossDamaged2.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastbossDamaged2.projectile.type"), config.getParameter("ffs1_lastbossDamaged2.projectile.config"), config.getParameter("ffs1_lastbossDamaged2.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastbossDamaged2.bobTime"),
    bobHeight = config.getParameter("ffs1_lastbossDamaged2.bobHeight"),
    skillTime = config.getParameter("ffs1_lastbossDamaged2.skillTime"),
...
function ffs1_lastbossDamaged2.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastbossDamaged2.update(dt, stateData)
 animator.setAnimationState("movement", "visible")
 
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastbossDamaged2.projectileOffset")
...
function ffs1_lastbossDamaged2.leavingState(stateData)
end
