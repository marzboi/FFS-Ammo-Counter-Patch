--------------------------------------------------------------------------------
ffs1_lastboss4Attack = {}

function ffs1_lastboss4Attack.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastboss4Attack.projectile.type"), config.getParameter("ffs1_lastboss4Attack.projectile.config"), config.getParameter("ffs1_lastboss4Attack.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastboss4Attack.bobTime"),
    bobHeight = config.getParameter("ffs1_lastboss4Attack.bobHeight"),
    skillTime = config.getParameter("ffs1_lastboss4Attack.skillTime"),
...
function ffs1_lastboss4Attack.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastboss4Attack.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "2Attack")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastboss4Attack.projectileOffset")
...
function ffs1_lastboss4Attack.leavingState(stateData)
end
