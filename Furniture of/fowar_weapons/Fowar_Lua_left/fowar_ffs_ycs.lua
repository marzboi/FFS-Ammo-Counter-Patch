...
function GunFire:auto()
  ...
  animator.playSound("fireBolt_fire")
  ...
  util.wait(self.stances.motion10.duration, function()
    ...
    local to = self.stances.reloadmotion1.weaponOffset or {0,0}
    self.weapon.weaponOffset = {interp.linear(progress, from[1], to[1]), interp.linear(progress, from[2], to[2])}

    self.weapon.relativeWeaponRotation = util.toRadians(interp.linear(progress, self.stances.motion10.weaponRotation, self.stances.reloadmotion1.weaponRotation))
    self.weapon.relativeArmRotation = util.toRadians(interp.linear(progress, self.stances.motion10.armRotation, self.stances.reloadmotion1.armRotation))
  ...
  self:fireProjectile("ffs_nothing")
  status.addEphemeralEffect("ffs_concentration")
  ...
  activeItem.setCursor("/cursors/ffs_reticle_reload_sr.cursor")
