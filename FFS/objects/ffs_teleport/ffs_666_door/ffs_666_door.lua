...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          name = "^yellow;666th Basement - Ruin's Hatchery",
          planetName = "^white;Hidden co-op dungeon - ^red;Very Hard",
          icon = "ffs_Num_666",
          warpAction = "Return"
        } }
      }
    ...
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          ...
          planetName = "^white;Hidden co-op dungeon - ^red;Very Hard",
          icon = "ffs_Num_666",
          warpAction = string.format("InstanceWorld:ffs0_666:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
