...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Whitecrow ^green;Bridge",
          planetName = "Col.Xander, Whitecrow Crews",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs_whitecrow_bridge:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
