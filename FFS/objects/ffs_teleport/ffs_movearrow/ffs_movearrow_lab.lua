...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Whitecrow ^green;Laboratory",
          planetName = "Maj.Lily, Lt.Alice, Dr.Breadnut",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs_whitecrow_lab:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
