...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Whitecrow ^green;Barrack",
          planetName = "Sgt.Max, Pvt.Jessie",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs_whitecrow_barrack:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
