...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Fight for recovering White crow",
          planetName = "Search and rescue survivors!",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs2_f2:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
