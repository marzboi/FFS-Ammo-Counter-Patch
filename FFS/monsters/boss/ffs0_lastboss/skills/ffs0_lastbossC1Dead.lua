...
function ffs0_lastbossC1Dead.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs0_lastbossC1Dead.projectile.type"), config.getParameter("ffs0_lastbossC1Dead.projectile.config"), config.getParameter("ffs0_lastbossC1Dead.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs0_lastbossC1Dead.bobTime"),
    bobHeight = config.getParameter("ffs0_lastbossC1Dead.bobHeight"),
    skillTime = config.getParameter("ffs0_lastbossC1Dead.skillTime"),
...
function ffs0_lastbossC1Dead.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs0_lastbossC1Dead.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "visible")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs0_lastbossC1Dead.projectileOffset")
...
function ffs0_lastbossC1Dead.leavingState(stateData)
end
