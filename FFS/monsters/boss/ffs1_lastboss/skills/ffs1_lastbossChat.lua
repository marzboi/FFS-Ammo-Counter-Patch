...
function ffs1_lastbossChat.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs1_lastbossChat.projectile.type"), config.getParameter("ffs1_lastbossChat.projectile.config"), config.getParameter("ffs1_lastbossChat.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs1_lastbossChat.bobTime"),
    bobHeight = config.getParameter("ffs1_lastbossChat.bobHeight"),
    skillTime = config.getParameter("ffs1_lastbossChat.skillTime"),
...
function ffs1_lastbossChat.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs1_lastbossChat.update(dt, stateData)
 animator.setAnimationState("movement", "visible")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs1_lastbossChat.projectileOffset")
...
function ffs1_lastbossChat.leavingState(stateData)
end
