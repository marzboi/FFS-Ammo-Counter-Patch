...
function ffs2_6bossChat_1.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_1.projectile.type"), config.getParameter("ffs2_6bossChat_1.projectile.config"), config.getParameter("ffs2_6bossChat_1.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_1.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_1.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_1.skillTime"),
...
function ffs2_6bossChat_1.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_1.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_1.projectileOffset")
...
function ffs2_6bossChat_1.leavingState(stateData)
end
