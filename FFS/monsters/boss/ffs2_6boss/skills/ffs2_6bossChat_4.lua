...
function ffs2_6bossChat_4.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("ffs2_6bossChat_4.projectile.type"), config.getParameter("ffs2_6bossChat_4.projectile.config"), config.getParameter("ffs2_6bossChat_4.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("ffs2_6bossChat_4.bobTime"),
    bobHeight = config.getParameter("ffs2_6bossChat_4.bobHeight"),
    skillTime = config.getParameter("ffs2_6bossChat_4.skillTime"),
...
function ffs2_6bossChat_4.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function ffs2_6bossChat_4.update(dt, stateData)
 status.addEphemeralEffect("invulnerable")
 animator.setAnimationState("movement", "talk")
	
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("ffs2_6bossChat_4.projectileOffset")
...
function ffs2_6bossChat_4.leavingState(stateData)
end
