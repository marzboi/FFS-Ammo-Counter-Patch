...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Retreat to White crow",
          planetName = "Max, Jessie, retreat, Retreat!",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs2_lastboss_1:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
