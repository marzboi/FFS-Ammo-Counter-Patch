...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Whitecrow ^green;Cargo bay",
          planetName = "SSG.Rebecca, PFC.Nemui, Yuri",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs_whitecrow_cargo:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
