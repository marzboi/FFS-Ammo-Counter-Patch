...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    return { "OpenTeleportDialog", {
        ...
        destinations = { {
          name = "Arrow",
          planetName = "Return to previous area.",
          icon = "ffs_Num_W",
          warpAction = "Return"
        } }
      }
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "2-6-C Save Point",
          planetName = "Break time!",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs2_6c:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
