...
function onInteraction(args)
  if config.getParameter("returnDoor") then
    ...
    return { "OpenTeleportDialog", {
        ...
        includePlayerBookmarks = false,
        destinations = { {
          name = "Whitecrow ^green;Infirmary",
          planetName = "Chiquita, SRS Agents",
          icon = "ffs_Num_W",
          warpAction = string.format("InstanceWorld:ffs_whitecrow_infirmary:%s:1", storage.uuid, world.threatLevel())
        } }
      }
    }
  end
